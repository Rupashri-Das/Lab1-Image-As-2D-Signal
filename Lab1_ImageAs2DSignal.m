...
% -------------------------------------------------------------------------
% HOW TO SUBMIT:
% Include screenshots and short explanations for each section in the GitHub repo.
...
if exist('peppers.png','file')
    ...
    I_rgb = repmat(imread('cameraman.tif'),1,1,3); % fallback image
end
...
title('Gamma: Original | γ=0.6 | γ=1.6');

%% 4) Sampling and aliasing (downsample then upsample)
scale = 0.1; % 10% of original size
I_small = imresize(I, scale, 'nearest');  % downsample
I_back = imresize(I_small, size(I), 'nearest'); % upsample
...
% 1) How does bit-depth affect visible banding or posterization?
% 2) How does contrast stretching change the histogram and visibility of details?
% 3) Why does aggressive downsampling cause aliasing (hint: Nyquist)?
